...
(function (jQuery) {
  ...
  for (let img of img_content) {
    img.addEventListener('click', (e) => {
      let img_info_Content = document.querySelectorAll(
        '.pq-map-info .pq-contact-boxes .pq-contact-box'
      );
      for (let content of img_info_Content) {
        if (
          content.getAttribute('data-number') ===
          img.getAttribute('data-number')
        ) {
...
const dots = document.querySelectorAll('.de-pagination-dot');
const totalSlides = slides.length;
let autoplayInterval;

function showSlide(index) {
  slides.forEach((slide, i) => {
    slide.classList.remove('de-active');
    dots[i].classList.remove('de-active');
  });

  currentSlide = (index + totalSlides) % totalSlides;
  slides[currentSlide].classList.add('de-active');
  dots[currentSlide].classList.add('de-active');
}
...
document.querySelector('.de-next-btn').addEventListener('click', () => {
  nextSlide();
  stopAutoplay();
  startAutoplay();
});

document.querySelector('.de-prev-btn').addEventListener('click', () => {
  prevSlide();
  stopAutoplay();
  startAutoplay();
});
